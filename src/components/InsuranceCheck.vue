<template>
    <div dir="rtl" class="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
      <h2 class="text-xl font-bold text-gray-800 mb-4">🚗 گزارش وضعیت بیمه‌ها</h2>
  
      <table v-if="expiringInsurances.length" class="w-full border-collapse border border-gray-200 shadow-sm">
        ...
        <tbody>
          <tr v-for="car in expiringInsurances" :key="car.plate" class="hover:bg-gray-50">
            ...
            <td class="p-3 border font-bold" :class="isExpired(car.expireInsurance) ? 'text-red-500' : 'text-yellow-500'">
              {{ isExpired(car.expireInsurance) ? "🚨 منقضی شده" : "⚠️ در حال انقضا" }}
            </td>
          </tr>
        </tbody>
      </table>
  
      <p v-else class="text-green-600 font-semibold">✅ همه بیمه‌ها معتبر هستند.</p>
    </div>
  </template>
  ...
  const { cars } = storeToRefs(useStore());
