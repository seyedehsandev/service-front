        ...
        >
          ...
          >
            <span class="text-gray-600">
              تعداد کل خودروها:
              <span class="font-bold text-blue-600">{{ carCount }}</span>
            </span>
          </div>
        </main>
...
function submitting() {
  if (editingCarId.value !== null) {
    updateCar({ ...carData, id: editingCarId.value, lastEdit: new Date() });
  } else {
...
function editData(carItem) {
  isFormOpen.value = true;
  editingCarId.value = carItem.id;
  populateForm(carItem);
}

function DeleteItem(carItem) {
  if (confirm(`آیا از حذف ${carItem.model} ${carItem.brand} مطمئن هستید؟`)) {
